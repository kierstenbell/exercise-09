...
```{r, 3}
# Renaming variables for easier pulling
ECV <- d$ECV
Group_size <- d$Group_size

# Solve for Beta1 by hand
(B1 <- cor(ECV, Group_size, use = "complete.obs") * (sd(ECV, na.rm = TRUE)/sd(Group_size, na.rm = TRUE)))

# Solve for Beta0 by hand 
(B0 <- mean(Group_size, na.rm = TRUE) - B1*mean(ECV, na.rm = TRUE)) 
```
